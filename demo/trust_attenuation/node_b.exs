...
store = fn cap -> :ok = CommitStore.store_capability(CommitStore, cap); cap end
...
{:ok, h_root_alice} = Capability.delegate(root.ctx, alice.keyed, claim.([:write, :delegate], ["d1"], unbounded))
store.(h_root_alice)
{:ok, h_alice_bob} = Capability.delegate(alice.ctx, bob.keyed, claim.([:write], ["d1"], unbounded), h_root_alice.id, parent: h_root_alice)
store.(h_alice_bob)

# 2. FORGED-sig leaf over d2.
{:ok, f_root_alice} = Capability.delegate(root.ctx, alice.keyed, claim.([:write, :delegate], ["d2"], unbounded))
store.(f_root_alice)
{:ok, f_alice_bob} = Capability.delegate(alice.ctx, bob.keyed, claim.([:write], ["d2"], unbounded), f_root_alice.id, parent: f_root_alice)
...
{:ok, o_root_alice} = Capability.delegate(root.ctx, alice.keyed, claim.([:write, :delegate], ["d3"], unbounded))
store.(o_root_alice)
overbroad =
  Capability.new(alice.keyed, bob.keyed, claim.([:write, :execute], ["d3"], unbounded), o_root_alice.id)
...
{:ok, s_root_alice} = Capability.delegate(root.ctx, alice.keyed, claim.([:write, :delegate], ["d4"], unbounded))
store.(s_root_alice)
{:ok, s_alice_bob} = Capability.delegate(alice.ctx, bob.keyed, claim.([:write], ["d4"], unbounded), s_root_alice.id, parent: s_root_alice)
store.(s_alice_bob)

# 5. EXPIRED leaf over d5: valid narrowing at mint, but not_after in the past.
past = DateTime.add(DateTime.utc_now(), -3600, :second)
{:ok, e_root_alice} = Capability.delegate(root.ctx, alice.keyed, claim.([:write, :delegate], ["d5"], unbounded))
store.(e_root_alice)
{:ok, e_alice_bob} = Capability.delegate(alice.ctx, bob.keyed, claim.([:write], ["d5"], %{not_before: nil, not_after: past}), e_root_alice.id, parent: e_root_alice)
store.(e_alice_bob)
...
bob_commit =
  Commit.new("d1", update, gen.id, %{kind: :regular, snapshot_parent: gen.id, capability_proof: h_alice_bob.id})
  |> Signing.sign_commit(bob.priv, bob.signer)

:ok = CommitStore.import_commit(CommitStore, bob_commit)

all_cert_cids =
  [h_root_alice, h_alice_bob,
   f_root_alice, forged,
   o_root_alice, overbroad,
   s_root_alice, s_alice_bob,
   e_root_alice, e_alice_bob]
  |> Enum.map(&hexid.(&1.id))
...
manifest = %{
  ...
  "attacks" => [
    %{"label" => "forged-sig cert", "doc" => "d2", "leaf" => hexid.(forged.id), "signer" => "bob", "expect" => "invalid_signature"},
    %{"label" => "over-broad cert (narrowing violation)", "doc" => "d3", "leaf" => hexid.(overbroad.id), "signer" => "bob", "expect" => "not_attenuation"},
    %{"label" => "stolen chain (commit-author binding)", "doc" => "d4", "leaf" => hexid.(s_alice_bob.id), "signer" => "mallory", "expect" => "capability_author_mismatch"},
    %{"label" => "expired cert", "doc" => "d5", "leaf" => hexid.(e_alice_bob.id), "signer" => "bob", "expect" => "expired"}
  ]
}
