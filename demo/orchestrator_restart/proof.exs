...
say.(
  "booting the :commonplace application (orchestrator_on_boot: true, scratch workspace #{dir})"
)
...
if orch1 == nil,
  do:
    (
      say.("FAIL — orchestrator not started")
      System.halt(1)
    )
...
put_doc = fn filename, content ->
  ...
  CommitStore.create_chained_commit(CommitStore, uuid, Yelixer.Encoding.encode_update(doc), %{
    kind: :regular
  })
...
put_doc.(
  "__processes.json",
  Jason.encode!(%{"proof_sleeper" => %{"mode" => "elixir", "source" => "proof.exs"}})
)
...
if p1 == nil,
  do:
    (
      say.("FAIL — proof_sleeper never started")
      System.halt(1)
    )
...
say.(
  ">>> KILLING the orchestrator (Process.exit(_, :kill)) — pre-move-#2 this was unrecoverable <<<"
)
...
if orch2 == nil,
  do:
    (
      say.("FAIL — supervisor did not restart the orchestrator")
      System.halt(1)
    )
...
if p2 == nil,
  do:
    (
      say.("FAIL — proof_sleeper not re-reconciled")
      System.halt(1)
    )
...
if p1_dead and p2 != p1 do
  ...
  say.(
    "PROOF PASSED — crash → supervisor restart → sweep (no duplicate generation) → re-reconcile."
  )
