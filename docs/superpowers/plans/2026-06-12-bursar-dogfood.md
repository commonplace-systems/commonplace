...
- [ ] Failing tests: (a) renew creates NO new commit on `__bursar.json`/`__bursar.log`;
...
- [ ] Failing tests: local delegation for all 7 verbs (against a named test Bursar);
      no-bursar-anywhere → `{:error, :bursar_unavailable}` for acquire/release/renew.
- [ ] Implement: mirrors CommitStoreClient — `remote_node/0` from
...
- [ ] Failing tests: move acquires both dir tokens sorted/deduped and releases on success,
...
- [ ] Implement; delete `@global_name` + GenServer machinery; update World.move docs.
- [ ] Full MUD suite green; commit.
...
- [ ] Failing tests: not-leader → `tick_now` returns `:not_leader`, no red events;
...
- [ ] Implement: delete `@global_name` (local `__MODULE__` default); state gains
...
- [ ] Failing test: `bursar_children/0` empty by default; non-empty when
      `:bursar_on_boot` true AND workspace root resolves.
- [ ] Implement gate (mirror `orchestrator_children/0`); serve sets
      `:bursar_on_boot` before boot; confirm test env stays bursar-free.
- [ ] Full core suite green (FOREGROUND — background runs hang on SandboxExecRunner
...
- [ ] Demo: serve with bursar; bare `mix run` temp node joins (THE incident);
...
- [ ] Commit artifact; report to boss; relay closes to commonplace worker.
