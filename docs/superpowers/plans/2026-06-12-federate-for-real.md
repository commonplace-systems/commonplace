...
| D9 | Who signs an agent's identity-doc registration in strict mode? | **DECIDED (jes, 2026-06-12): the CREATOR signs** — the principal who mints + delegates to the agent signs its registration commits (accountability chains: same principal then `cap delegate`s the scoped cert). Node-sign remains the fallback for headless contexts. Implementation: optional `signing_context` opt threaded through `Presence.Identity`. |
