...
| `COMMONPLACE_LOCAL_READ_GATE` / `:local_read_gate` | `:permissive` | Serve reads without applying the authorization gate. | Current behavior: `String.to_atom/1` mints the value; the next gated read raises `CaseClauseError` because it is outside `permissive \| dry_run \| enforce`. Boot itself does not refuse. | `apps/commonplace/lib/commonplace/trust/read.ex:188`; bridge `config/runtime.exs:152` |
