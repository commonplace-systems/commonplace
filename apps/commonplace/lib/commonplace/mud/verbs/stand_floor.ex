defmodule Commonplace.MUD.Verbs.StandFloor do
  @moduledoc """
  CX-wkau (MUD-as-documents Inc-1, tranche 1) — the compiled-in FLOOR for the
  doc-hosted `stand` verb, mirroring `Commonplace.MUD.Verbs.LookFloor`'s role
  ...
  from any doc-hosted `stand` module) when no manifest entry is set, when
  ...
  `run/2` is a one-line delegator to `Commonplace.MUD.Verbs.do_stand/2` (via
  the `__stand_floor__/2` escape hatch) — NOT a reimplementation. This means
  the floor can never drift out of parity with the live compiled-in verb:
  there is exactly one implementation of `stand`, this just exposes a call
  ...
  def run(cmd, ctx), do: Verbs.__stand_floor__(cmd, ctx)
end
