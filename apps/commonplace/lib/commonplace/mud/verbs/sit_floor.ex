defmodule Commonplace.MUD.Verbs.SitFloor do
  @moduledoc """
  CX-wkau (MUD-as-documents Inc-1, tranche 1) — the compiled-in FLOOR for the
  doc-hosted `sit` verb, mirroring `Commonplace.MUD.Verbs.LookFloor`'s role
  ...
  from any doc-hosted `sit` module) when no manifest entry is set, when
  ...
  `run/2` is a one-line delegator to `Commonplace.MUD.Verbs.do_sit/2` (via
  the `__sit_floor__/2` escape hatch) — NOT a reimplementation. This means
  the floor can never drift out of parity with the live compiled-in verb:
  there is exactly one implementation of `sit`, this just exposes a call
  ...
  def run(cmd, ctx), do: Verbs.__sit_floor__(cmd, ctx)
end
