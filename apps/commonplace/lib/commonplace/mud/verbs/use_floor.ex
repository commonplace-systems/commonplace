defmodule Commonplace.MUD.Verbs.UseFloor do
  @moduledoc """
  CX-wkau (MUD-as-documents Inc-1, tranche 2) — the compiled-in FLOOR for the
  doc-hosted `use` verb, mirroring `Commonplace.MUD.Verbs.WhereFloor`'s role
  ...
  from any doc-hosted `use` module) when no manifest entry is set, when
  ...
  `run/2` is a one-line delegator to `Commonplace.MUD.Verbs.do_use/2` (via
  the `__use_floor__/2` escape hatch) — NOT a reimplementation. This means
  the floor can never drift out of parity with the live compiled-in verb:
  there is exactly one implementation of `use`, this just exposes a call
  ...
  def run(cmd, ctx), do: Verbs.__use_floor__(cmd, ctx)
end
