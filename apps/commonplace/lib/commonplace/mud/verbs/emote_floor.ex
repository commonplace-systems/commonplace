defmodule Commonplace.MUD.Verbs.EmoteFloor do
  @moduledoc """
  CX-aya0 (MUD-as-documents Inc-2 / B2) — the compiled-in FLOOR for the
  doc-hosted `emote` verb, mirroring `Commonplace.MUD.Verbs.LookFloor`'s
  ...
  from any doc-hosted `emote` module) when no manifest entry is set, when
  ...
  `run/2` is a one-line delegator to `Commonplace.MUD.Verbs.do_emote/2`
  (via the `__emote_floor__/2` escape hatch) — NOT a reimplementation.
  This means the floor can never drift out of parity with the live
  compiled-in verb: there is exactly one implementation of `emote`, this
  ...
  def run(cmd, ctx), do: Verbs.__emote_floor__(cmd, ctx)
end
