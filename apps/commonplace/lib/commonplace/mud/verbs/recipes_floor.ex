defmodule Commonplace.MUD.Verbs.RecipesFloor do
  @moduledoc """
  CX-wkau (MUD-as-documents Inc-1, tranche 2) — the compiled-in FLOOR for the
  doc-hosted `recipes` verb, mirroring `Commonplace.MUD.Verbs.WhereFloor`'s
  role for `where` (tranche 1). `EngineModule.resolve/2` falls back to this
  module (never purged by a `SourceDoc` recompile — it is a DISTINCT module
  from any doc-hosted `recipes` module) when no manifest entry is set, when
  ...
  `run/2` is a one-line delegator to `Commonplace.MUD.Verbs.do_recipes/2`
  (via the `__recipes_floor__/2` escape hatch) — NOT a reimplementation.
  This means the floor can never drift out of parity with the live
  compiled-in verb: there is exactly one implementation of `recipes`, this
  ...
  def run(cmd, ctx), do: Verbs.__recipes_floor__(cmd, ctx)
end
