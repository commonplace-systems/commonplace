defmodule Commonplace.MUD.Verbs.SayFloor do
  @moduledoc """
  CX-aya0 (MUD-as-documents Inc-2 / B2) — the compiled-in FLOOR for the
  doc-hosted `say` verb, mirroring `Commonplace.MUD.Verbs.LookFloor`'s role
  ...
  from any doc-hosted `say` module) when no manifest entry is set, when the
  ...
  `run/2` is a one-line delegator to `Commonplace.MUD.Verbs.do_say/2` (via
  the `__say_floor__/2` escape hatch) — NOT a reimplementation. This means
  the floor can never drift out of parity with the live compiled-in verb:
  there is exactly one implementation of `say`, this just exposes a call
  ...
  def run(cmd, ctx), do: Verbs.__say_floor__(cmd, ctx)
end
