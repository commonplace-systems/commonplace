defmodule Commonplace.MUD.Verbs.ExamineFloor do
  @moduledoc """
  CX-wkau (MUD-as-documents Inc-1, tranche 1) — the compiled-in FLOOR for the
  doc-hosted `examine` verb, mirroring `Commonplace.MUD.Verbs.LookFloor`'s
  ...
  module from any doc-hosted `examine` module) when no manifest entry is
  ...
  `run/2` is a one-line delegator to `Commonplace.MUD.Verbs.do_examine/2`
  (via the `__examine_floor__/2` escape hatch) — NOT a reimplementation.
  This means the floor can never drift out of parity with the live
  compiled-in verb: there is exactly one implementation of `examine`, this
  ...
  def run(cmd, ctx), do: Verbs.__examine_floor__(cmd, ctx)
end
