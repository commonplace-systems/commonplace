defmodule Commonplace.MUD.CxK20zSandboxReproTest do
  ...
  setup do
    ...
    start_supervised!(
      {Commonplace.Store.Supervisor,
       data_dir: dir, name: :"k20z_sup_#{n}", commit_store_name: store,
       trust_side_store_name: :"k20z_tss_#{n}", pending_imports_name: :"k20z_pi_#{n}"}
    )

    old = {Application.get_env(:commonplace, :data_dir), Application.get_env(:commonplace, :trust), Application.get_env(:commonplace, :local_write_gate)}
    ...
    on_exit(fn ->
      ...
      if t, do: Application.put_env(:commonplace, :trust, t), else: Application.delete_env(:commonplace, :trust)
      if k, do: Application.put_env(:commonplace, :local_write_gate, k), else: Application.delete_env(:commonplace, :local_write_gate)
      File.rm_rf!(dir)
    end)
  ...
  defp mk_room!(store, sc, name) do
    {:ok, u} = Schemas.create_dir_with_meta(Schemas.room_filename(), Schemas.encode_room(%Room{name: name, description: "a place"}), store, signing_context: sc)
    u
  end
  ...
  defp visitor_facade(room, root, store) do
    ...
    ctx = %{signing_context: vctx, cert_cids: [], signer_id: nil, current_room_uuid: room, root_uuid: root, player_name: "v", player_uuid: "vp", inventory_uuid: "vi"}
  ...
  test "DETERMINISTIC REPRO (fixture): the live 'start' pre-corruption CRDT state + one write_meta_doc → EMPTY", ctx do
    ...
    {m, o} = Commonplace.MUD.SignedWrite.opts_for(dir, store: ctx.store, signing_context: ctx.node_ctx)
    ...
    new_json = Jason.encode!(Jason.decode!(seeded_content) |> Map.update("exits", %{}, &Map.put(&1, "probeexit", ctx.commons)))
  ...
  test "EMPTY-INITIAL: create meta with EMPTY content, then fill, then re-write (mimics the stub→bootstrap 'start' history)", ctx do
    ...
    r1 = Schemas.encode_room(%Room{name: "The Start Room", description: "A snug antechamber — worn — the threshold.", exits: %{"north" => ctx.commons}})
    :ok = Schemas.write_meta_doc(meta, r1, ctx.store, signing_context: ctx.node_ctx)
    assert {:ok, %Room{name: "The Start Room"}} = World.get_room(dir, ctx.store)

    r2 = Schemas.encode_room(%Room{name: "The Start Room", description: "A snug antechamber — worn smooth — the threshold.", exits: %{"north" => ctx.commons, "south" => ctx.commons}})
  ...
  test "MINIMAL: two write_meta_doc delete-all-reinsert calls under the SAME funnel hand (signer_id nil) corrupts", ctx do
    ...
    :ok = Schemas.write_meta_doc(meta, Schemas.encode_room(%Room{name: "Seeded", description: "d1", exits: %{"north" => ctx.commons}}), ctx.store, signing_context: ctx.node_ctx)
    ...
    :ok = Schemas.write_meta_doc(meta, Schemas.encode_room(%Room{name: "Seeded", description: "d2", exits: %{"north" => ctx.commons, "south" => ctx.commons}}), ctx.store, signing_context: ctx.node_ctx)
  ...
  test "ACCUMULATED: N repeated funnel-hand write_meta_doc calls — at what N does it corrupt?", ctx do
    room = mk_room!(ctx.store, ctx.node_ctx, "Seeded")
    {:ok, meta} = World.meta_doc_uuid(room, Schemas.room_filename(), ctx.store)
    ...
    first_bad =
      Enum.reduce_while(1..30, nil, fn i, _ ->
        ...
        :ok = Schemas.write_meta_doc(meta, Schemas.encode_room(%Room{name: "Seeded", description: desc, exits: %{"north" => ctx.commons}}), ctx.store, signing_context: ctx.node_ctx)

        case World.get_room(room, ctx.store) do
          {:ok, %Room{description: ^desc}} -> {:cont, nil}
          other -> IO.puts("[k20z accum] CORRUPTED at write ##{i}: #{inspect(other) |> String.slice(0,100)}"); {:halt, i}
        end
      end)
  ...
  test "SANDBOX: open_exit run THROUGH run_safe_verb on a FRESH curated room — corrupts?", ctx do
    ...
    :ok = VerbSource.save_safe_verb(source, "reveal", body, [source], ctx.store, signing_context: ctx.node_ctx)
