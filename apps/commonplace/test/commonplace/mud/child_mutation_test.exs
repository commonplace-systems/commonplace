defmodule Commonplace.MUD.ChildMutationTest do
  ...
  test "create_zone_root SELF-ROOTS: child.zone == its own uuid", %{store: store, players: players} do
    assert {:ok, home} = ChildMutation.create_zone_root(players, "alice", Schemas.room_filename(), room_json("Alice's Home"), store)
  ...
  test "create_zoned_child INHERITS the parent's zone", %{store: store, players: players} do
    {:ok, home} = ChildMutation.create_zone_root(players, "bob", Schemas.room_filename(), room_json("Bob's Home"), store)

    # node builds a child under the home (node entry-add for this test)
    {:ok, node_ctx} = NodeIdentity.signing_context()

    assert {:ok, study} =
             ChildMutation.create_zoned_child(home, "study", Schemas.room_filename(), room_json("Study"), store, signing_context: node_ctx)

    assert Trust.doc_zone(study, store) == home
    assert entry?(home, "study", store)
  end

  test "un-zoned parent → un-stamped child (fail-closed-safe, no forged zone)", %{store: store, players: players} do
    # `players` itself is a bare node dir with no zone → a child under it inherits nil
    {:ok, node_ctx} = NodeIdentity.signing_context()

    assert {:ok, child} =
             ChildMutation.create_zoned_child(players, "loose", Schemas.room_filename(), room_json("Loose"), store, signing_context: node_ctx)

    assert Trust.doc_zone(child, store) == nil
  end

  test "PLAYER builds under their home via their {:subtree} cert (split authority)", %{store: store, node_ctx: node_ctx, players: players} do
    {:ok, home} = ChildMutation.create_zone_root(players, "carol", Schemas.room_filename(), room_json("Carol's Home"), store)
    ...
    assert {:ok, study} =
             ChildMutation.create_zoned_child(home, "study", Schemas.room_filename(), room_json("Study"), store, p.creds)
  ...
  test "a PLAYER cannot build OUTSIDE their zone (the entry-add is carve-rejected)", %{store: store, node_ctx: node_ctx, players: players} do
    {:ok, home1} = ChildMutation.create_zone_root(players, "dave", Schemas.room_filename(), room_json("Dave's Home"), store)
    {:ok, home2} = ChildMutation.create_zone_root(players, "erin", Schemas.room_filename(), room_json("Erin's Home"), store)
    ...
    assert {:error, _} =
             ChildMutation.create_zoned_child(home2, "sneak", Schemas.room_filename(), room_json("Sneak"), store, dave.creds)
