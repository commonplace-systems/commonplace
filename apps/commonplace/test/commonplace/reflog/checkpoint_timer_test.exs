defmodule Commonplace.Reflog.CheckpointTimerTest do
  ...
  setup do
    dir = Path.join(System.tmp_dir!(), "cp_timer_#{:rand.uniform(999999)}")
    File.mkdir_p!(dir)

    store_name = :"timer_store_#{:rand.uniform(999999)}"
  ...
  test "checkpoint_now creates a checkpoint", %{store_name: store_name, root_uuid: root_uuid} do
    {:ok, timer} = CheckpointTimer.start_link(
      root_uuid: root_uuid,
      store: store_name,
      owner: "test",
      interval: 60_000,
      name: :"timer_#{:rand.uniform(999999)}"
    )
  ...
  test "timer triggers periodic checkpoints", %{store_name: store_name, root_uuid: root_uuid} do
    {:ok, timer} = CheckpointTimer.start_link(
      ...
      owner: "test",
      interval: 100,
      name: :"timer_fast_#{:rand.uniform(999999)}"
    )
    ...
    schema = case CommitStore.latest_commit(store_name, root_uuid) do
      {:ok, commit} ->
