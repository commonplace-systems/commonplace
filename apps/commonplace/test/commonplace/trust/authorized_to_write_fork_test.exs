defmodule Commonplace.Trust.AuthorizedToWriteForkTest do
  ...
  setup do
    ...
    {:ok, cap} =
      Capability.issue(node_ctx, {pid, pub}, %{verbs: [:define_verb], scope: {:subtree, home}, caveats: %{}}, nil, store: store)
  ...
  defp mint_source(store, content, ctx, define_cid, home) do
    uuid = UUID.uuid4()
    doc = Yelixer.Doc.new() |> ContentType.create(:text, "verb") |> ContentType.insert_text(0, content)
  ...
  test "a VALID sandboxed safe-verb classifies :define_verb → the citizen IS authorized", %{store: store, home: home, citizen: citizen, define_cid: cid} do
  ...
  test "RAW unsafe code classifies :execute (Gate-B) → the citizen's :define_verb cert is DENIED", %{store: store, home: home, citizen: citizen, define_cid: cid} do
  ...
  test "the SAME raw code IS authorized when the NODE authors it (authority, not a broken classifier)", %{store: store, home: home, node_ctx: node_ctx, define_cid: cid} do
  ...
  test "a data write classifies :write → the define_verb-ONLY citizen is DENIED (no :write in the grant)", %{store: store, home: home, citizen: citizen, define_cid: cid} do
    {uuid, commit} = mint_source(store, "just some plain prose, not code at all", citizen, cid, home)
